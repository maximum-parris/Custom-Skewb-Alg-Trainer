...
function generateScramble() {
    ...
    if (recapArray.length == 0 && incorrectAlgs.length == 0) { // train
        ...
        if (recapFin == false) { // recap
            ...
            caseNum = randomElement(incorrectAlgs);

            const index = incorrectAlgs.indexOf(caseNum);
            incorrectAlgs.splice(index, 1);
        }
    }
...
function recapAgain() {
    if (!incorrectAlgs.includes(caseNum)) {
        incorrectAlgs.push(caseNum);
    }
}